...
var CustomOrder = React.createClass({displayName: "CustomOrder",
	...
	render: function () {
		...
		return (
			React.createElement("div", {className: "CustomOrder"}, 
...
CustomOrder.Order = React.createClass({displayName: "Order",
	...
	render: function () {
		...
		var stepViews = [
			React.createElement(CustomOrder.Days, React.__spread({key: "Days"},  props, {days: this.state.days, currentDayOption: this.state.currentDayOption, updateDays: this.updateDays, nextStep: this.nextStep})),
			React.createElement(CustomOrder.Packages, React.__spread({key: "Packages"},  props, {currentPackage: this.state.currentPackage, updatePackage: this.updatePackage, nextStep: this.nextStep})),
			React.createElement(CustomOrder.Customize, React.__spread({key: "Customize"},  props, {days: this.state.days, currentPackage: this.state.currentPackage, juiceSelections: this.state.juiceSelections, updateJuiceSelections: this.updateJuiceSelections, nextStep: this.props.completeOrder}))
		]

		return (
			React.createElement("div", {className: "Order"}, 
				React.createElement(CustomOrder.Navigation, {step: this.state.currentStep, setStep: this.setStep}), 
				React.createElement(ReactCSSTransitionGroup, {transitionName: "step", className: "StepViews", transitionEnterTimeout: 500, transitionLeaveTimeout: 300}, 
...
CustomOrder.Navigation = React.createClass({displayName: "Navigation",
	...
	render: function () {
		...
		var stepNodes = _.map(steps, function (step, index) {
			...
			return React.createElement("li", {key: "Step" + index, style: styles, className: classes, onClick: self.props.setStep.bind(null, index)}, React.createElement("span", null, step.label))
		})
		var progressStyle = {
		...
		return (
			React.createElement("div", {className: "Navigation"}, 
				React.createElement("div", {className: "mw-progress-bar"}, 
					React.createElement("div", {className: "mw-progress", style: progressStyle})
				), 
				React.createElement("ul", {className: "mw-steps"}, 
					stepNodes
				)
			)
		)
...
CustomOrder.Days = React.createClass({displayName: "Days",
	...
	render: function () {
			...
			if (dayOption.Days === "0") {
				description = (
					React.createElement("div", {className: "description"}, 
						React.createElement("input", {className: "input", type: "number", value: self.props.days, onChange: self.updateDays.bind(self,{ days: dayOption.Days, currentDayOption: index }), placeholder: "Choose # of Days"})
					)
				)
			} else {
				description = React.createElement("div", {className: "description"}, dayOption.Description)
			}
			return (
				React.createElement("div", {key: "days-" + dayOption.Days, className: columns}, 
					React.createElement("div", {className: optionClasses, onClick: self.updateDays.bind(self,{ days: dayOption.Days, currentDayOption: index })}, 
		...
		return (
			React.createElement("div", {className: "Days step-view"}, 
				React.createElement("header", null, 
					React.createElement("h3", null, "How many days will you cleanse?"), 
					React.createElement("p", {className: "instructions"}, "The most popular is a 3-day cleanse, but even a 1-day cleanse will help detoxify your system. The 5-day will cleanse at deep cellular level. It typically takes 2 days to get over the detox symptoms."), 
					React.createElement("p", {className: "pricing"}, "$65/Day")
				), 
				React.createElement("main", {className: "container"}, 
					React.createElement("div", {className: "day-options mw-row"}, 
						dayOptions
					)
				), 
				React.createElement("footer", null, 
					React.createElement("button", {onClick: this.props.nextStep}, "Next Select Juices ", React.createElement("i", {className: "icon mk-moon-arrow-right-6"}))
				)
			)
		)
	}
...
CustomOrder.Packages = React.createClass({displayName: "Packages",
	...
	render: function () {
		...
		var groups = _.map(this.props.data.groups, function (group, index) {
			...
			if (index > 0) {
				return (
					React.createElement("div", {key: "Group" + index, className: "group"}, 
						React.createElement("span", {className: "color", style: styles}), 
						React.createElement("span", {className: "mw-label"}, group.Description)
					)
				)
			}
		})
		var packageOptions = _.map(this.props.data.packages, function (packageOption, index) {
			...
			for (i=0; i < packageLength; i++) {
				...
				packageGroupItems[i] = React.createElement("span", {key: "PackageGroup" + i, className: "package-group", style: styles})
			}
			if (packageOption.Name !== "Custom") {
				packageGroups = (
					React.createElement("div", {className: "package-groups"}, 
			...
			return (
				React.createElement("div", {key: "Package" + index, className: columns}, 
					React.createElement("div", {className: "tag", style: tagStyle}), 
					React.createElement("div", {className: optionClasses, onClick: self.updatePackage.bind(self, index)}, 
						React.createElement("div", {className: "information", style: packageBackgroundStyle}, 
							React.createElement("div", {className: "name"}, packageOption.Name), 
							React.createElement("div", {className: "description"}, packageOption.Description)
						), 
						packageGroups
					)
				)
		...
		return (
			React.createElement("div", {className: "Packages step-view"}, 
				React.createElement("header", null, 
					React.createElement("h3", null, "Select your cleanse package"), 
					React.createElement("p", {className: "instructions"}, "These packages are great guides but have no fear you can customize as you go!")
				), 
				React.createElement("main", {className: "container"}, 
					React.createElement("div", {className: "package-options mw-row"}, 
						packageOptions
					), 
					React.createElement("div", {className: "group-legend"}, 
						groups
					)
				)
			)
...
CustomOrder.Customize = React.createClass({displayName: "Customize",
	...
	render: function () {
		...
		var dayViews = _.map(this.props.juiceSelections, function (dayJuices, index) {
			return (
				React.createElement(CustomOrder.Customize.Day, {key: "DayView" + index, data: self.props.data, currentPackage: self.props.currentPackage, updateJuiceSelections: self.props.updateJuiceSelections, currentDay: self.state.currentDay, juiceSelections: dayJuices})
			)
		})
		return (
			React.createElement("div", {className: "Customize step-view"}, 
				React.createElement("header", null, 
					React.createElement("h3", null, "Customize your juice selections"), 
					React.createElement("p", {className: "instructions"}, "Go ahead and change it up to your liking, but just remember that the greens have the most detoxify capabilities.")
				), 
				React.createElement("main", null, 
					React.createElement(CustomOrder.Customize.Navigation, React.__spread({},  this.props, {currentDay: this.state.currentDay, updateCurrentDay: this.updateCurrentDay})), 
					React.createElement(ReactCSSTransitionGroup, {transitionName: "day", className: "DayViews"}, 
						dayViews[this.state.currentDay]
					)
				), 
				React.createElement("footer", null, 
					React.createElement("button", {onClick: this.props.nextStep}, "Review & place order ", React.createElement("i", {className: "icon mk-moon-arrow-right-6"}))
				)
			)
		)
	}
...
CustomOrder.Customize.Navigation = React.createClass({displayName: "Navigation",
	...
	render: function () {
		...
		for (dayIndex = 0; dayIndex < this.props.days; dayIndex++) {
			...
			dayTabs[dayIndex] = React.createElement("div", {className: classes, style: styles, key: "DaySelector" + dayIndex, onClick: this.props.updateCurrentDay.bind(null, dayIndex)}, "Day ", dayIndex + 1)
		}
		return (
			React.createElement("div", {className: "CustomizeNavigation mw-row"}, 
				dayTabs
			)
		)
	}
...
CustomOrder.Customize.Day = React.createClass({displayName: "Day",
	...
	render: function () {
		...
		var juiceViews = _.map(this.props.juiceSelections, function (currentJuice, index) {
			return (
				React.createElement(CustomOrder.Customize.Juice, {data: self.props.data, currentDay: self.props.currentDay, currentPackage: self.props.currentPackage, updateJuiceSelections: self.props.updateJuiceSelections, currentJuice: currentJuice, juiceIndex: index})
			)
		})
		return (
			React.createElement("div", {key: "Day" + self.props.currentDay, className: "CustomizeDay mw-row"}, 
...
CustomOrder.Customize.Juice = React.createClass({displayName: "Juice",
	...
	render: function () {
		...
		var juiceOptions = _.map(_.filter(self.props.data.juices, function (juice) { return (juice.Group == currentGroup.Name) || currentGroup.Name == "All" }), function (juiceOption, index) {
			...
			return (
				React.createElement("li", {key: "JuiceOpion" + index, className: classes, onClick: self.updateJuice.bind(null, juiceOption.Name)}, juiceOption.Name, " ", priceAdjustString)
			)
		})
		var classes = [
			"Juice",
		...
		return (
			React.createElement("div", {className: "column-small-4 column-medium-2"}, 
				React.createElement("div", {className: classes}, 
					React.createElement("div", {className: "image", onClick: this.toggleOptions}, React.createElement("img", {src: this.props.currentJuice.Image, alt: this.props.currentJuice.Name, className: "bottle"})), 
					React.createElement("div", {className: "information"}, 
						React.createElement("div", {className: "name"}, this.props.currentJuice.Name), 
						React.createElement("div", {className: "ingredients"}, this.props.currentJuice.Ingredients)
					), 
					React.createElement("div", {className: "juice-selector"}, 
