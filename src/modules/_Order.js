    ...
    return (
      <div className="Navigation">
        <div className="mw-progress-bar">
          <div className="mw-progress" style={progressStyle} />
        </div>
        <ul className="mw-steps">{stepNodes}</ul>
      </div>
    );
  }
});
  ...
  render: function() {
    ...
    var dayOptions = _.map(this.props.data.days, function(dayOption, index) {
      ...
      if (dayOption.Days === "0") {
        ...
        description = (
          <div className="description">{dayOption.Description}</div>
        );
      }
      return (
      ...
      <div className="Days step-view">
        <header>
          ...
          <p className="instructions">
            The most popular is a 3-day cleanse, but even a 1-day cleanse will
            help detoxify your system. The 5-day will cleanse at deep cellular
            level. It typically takes 2 days to get over the detox symptoms.
          </p>
          <p className="pricing">$65/Day</p>
        </header>
        <main className="container">
          <div className="day-options mw-row">{dayOptions}</div>
        </main>
        <footer>
          <button onClick={this.props.nextStep}>
            Next Select Juices <i className="icon mk-moon-arrow-right-6" />
          </button>
        </footer>
      </div>
    );
...
CustomOrder.Packages = React.createClass({
  ...
  render: function() {
      ...
      if (index > 0) {
        return (
          <div key={"Group" + index} className="group">
            <span className="color" style={styles} />
    ...
    var packageOptions = _.map(this.props.data.packages, function(
      ...
      console.log(
        "packageLength in Custom Order Packaged (_Order.js)",
        packageLength
      );
      ...
      if (packageOption.Name !== "Custom") {
        ...
        packageGroups = <div className="package-groups">&nbsp;</div>;
      }
    ...
    return (
      <div className="Packages step-view">
        <header>
          ...
          <p className="instructions">
            These packages are great guides but have no fear you can customize
            as you go!
          </p>
        </header>
        <main className="container">
    ...
    return (
      <div className="Customize step-view">
        <header>
          ...
          <p className="instructions">
            Go ahead and change it up to your liking, but just remember that the
            greens have the most detoxify capabilities.
          </p>
        </header>
        <main>
        ...
        <footer>
          <button onClick={this.props.nextStep}>
            Review &amp; place order{" "}
            <i className="icon mk-moon-arrow-right-6" />
          </button>
        </footer>
      </div>
    );
...
CustomOrder.Customize.Navigation = React.createClass({
  ...
  render: function() {
    ...
    return <div className="CustomizeNavigation mw-row">{dayTabs}</div>;
  }
});
    ...
    var juiceOptions = _.map(
      ...
      function(juiceOption, index) {
        ...
        return (
          ...
          >
            {juiceOption.Name} {priceAdjustString}
          </li>
        );
      }
    );
    ...
    return (
      <div className="column-small-4 column-medium-2">
        <div className={classes}>
          <div className="image" onClick={this.toggleOptions}>
            <img
          ...
          <div className="juice-selector">
            <ul className="options">{juiceOptions}</ul>
          </div>
        </div>
      </div>
    );
